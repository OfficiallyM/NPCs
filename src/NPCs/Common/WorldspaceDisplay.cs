...
namespace NPCs.Common
{
	internal class WorldspaceDisplay : MonoBehaviour
	{
		public class Message
		{
			...
			public Message(List<string> rows, float typewriterSpeed = 0.03f)
			{
				Rows = rows;
		...
		private void ResizeCanvasForContent(List<string> rows, float padding = 20f)
		{
			...
			{
				Vector2 unconstrained = _text.GetPreferredValues(row, Mathf.Infinity, Mathf.Infinity);
				float clampedWidth = Mathf.Min(unconstrained.x + padding, _maxWidth);

				// Get the actual height TMP would need at the clamped width, accounting for wrapping.
				Vector2 constrained = _text.GetPreferredValues(row, clampedWidth - padding, Mathf.Infinity);

				if (clampedWidth > totalWidth)
					totalWidth = clampedWidth;

				totalHeight += constrained.y;
			}

			// Add padding and a small buffer per row for the line spacing between rows.
			totalHeight += padding + (rows.Count * _text.fontSize * 0.2f);

			RectTransform canvasRect = _canvas.GetComponent<RectTransform>();
			canvasRect.sizeDelta = new Vector2(totalWidth, totalHeight);
		}

		private IEnumerator DisplayRoutine(List<string> rows, float typewriterSpeed)
		{
			_text.text = string.Empty;
